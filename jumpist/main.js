...
const FHEIGHT = 10000;
let DEBUG = !false// Boolean(confirm("debug mode"));
var config = {
    type: Phaser.AUTO,
    width: WIDTH,
    height: HEIGHT,
...
*/
var score_elt = document.querySelector("p[score]");
var best_score_elt = document.querySelector("p[best-score]");
var player;
var stars;
var bombs;
var platforms;
...
try {
    if(typeof eval(localStorage.best) != "number") throw "error";
}
catch(e) {
    localStorage.best = "0";
}
...
function create ()
{
    ...
    bomb_ys = []
    for(var i = 0;i < platforms_coords.length; i += 5) {
        bomb_ys.push(platforms_coords[i].y);
    }
    ...
    best_score_elt.textContent = "best score: "+localStorage.best+"\nBy: "+localStorage.bestname;
...
function update ()
{
    if (gameOver)
    {
        while(!name && localStorage.best < score) {
            name = prompt("enter your name");
            if(!name) continue;
            console.log("score: ", score)
            localStorage.best = score;
            localStorage.bestname = name;
        }
        return;
    }
    ...
    for(var y of bomb_ys) {
        if(y>(player.y-HEIGHT/2)) {
            var bomb = bombs.create(((player.x < 400) ? Phaser.Math.Between(400, 800) : Phaser.Math.Between(0, 400)), y, 'bomb');
            bomb.setBounce(1);
...
function collectStar (player, star)
{
    ...
    //score += Math.pow(2, nbombs);
    //score_elt.textContent = 'Score: ' + score;
    
        //  A new batch of stars to collect
    setTimeout(function() {
        star.enableBody(true, star.x, star.y-1000, true, true);        
    }, 1000);
    

        /*var x = (player.x < 400) ? Phaser.Math.Between(400, 800) : Phaser.Math.Between(0, 400);
        var bomb = bombs.create(x, GameY, 'bomb');
        bomb.setBounce(1);
        bomb.setVelocity(Phaser.Math.Between(-200, 200), 20);
        bomb.allowGravity = false;
        nbombs += 1;*/
